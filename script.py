...
def has_Q(tweet):
	   if (countMe(tweet, "?"))==0:
		   ...
		   return True
def has_A(tweet,Mark):
	   if (countMe(tweet, Mark))==0:
		   return False
	   else:
		   return True

print ("Total tweets found in dataset: "+str(len(tweets)))
...
for tweet in tweets:
	...
	is_empty_mention = is_empty(tweet["usermentions"])
	is_empty_description = is_empty(tweet["description"])
	has_urls = is_empty(tweet["urls"])
	has_hashtag = is_empty(tweet["hashtag"])
	is_empty_user_url= is_empty("url")
	q1 = countMe(tweet["text"], "?")
	count_Q = q1
	is_replytostatus= is_empty(tweet["inreplytostatusid"])
	count_screenname,count_uniqe_screenname=countWords(tweet["screenname"])
	count_description,count_uniqe_description=countWords(tweet["description"])
	...
	value.append(is_empty_mention)                          #has user mention
	value.append(retweetcount)                              #retweetcount
	value.append(countMe(tweet["text"], "RT"))              # tweet is retweet
	value.append(is_replytostatus)                          # tweet is replay
	...
	value.append(countMe(tweet["text"], "www"))               # count of URL
	value.append(countMe(tweet["text"], "#"))               # count of hashtagsvalue.append(count_Q))
	value.append(count_Q) 		                         	#count of question marks
	...
	value.append(has_urls)                                   #has Url
	value.append(has_hashtag)                                #has hashtag
	value.append(has_Q(tweet["text"]))                       #has questionmark
	value.append(has_A(tweet["text"], "!"))                   #has execlamationmark
	value.append(has_A(tweet["text"], "..."))                 #has ellipses
	value.append(has_A(tweet["text"], "$"))                   # has stock symbol
	value.append(hasUrlShortner(tweet["text"]))               # used url shortnel
# ##### end of 24 content based features//except the sentement analysis
	value.append(tweet["followerscount"])                      #count of follower
	value.append(tweet["friendcount"])                      #count of friends
	value.append(fo_fe)										#follower/friend ratio of author
	value.append(fe_fo)										#friend/follower ratio of author
	value.append(tweet["verified"])                         #is verefied
	value.append(is_empty_description)                  # has user descreption
	value.append(count_description)      #length of user descreption
	value.append(is_empty_user_url)                  # has user url
	value.append(tweet["defaultprofileimage"])         # has defult profile image
	value.append(detectFace(tweet["imageurlhttps"]))		#detecting if author's image holds a face
	value.append(count_screenname)      #length of secreenname
	value.append(calculateAuthorAge(tweet["creationdate"],tweet["creationdate1"])) 	#Registration age of author in days
	value.append(tweet["statusescount"])         # status count
	value.append(tweet["favoritecount"])         # favoritecount
	value.append(retweetFraction)						#retweet fraction of the author
	value.append(tweetTimeSpacing)                                                   # average time spacing of author in days
	...
	value.append(avgTweetLength)  # Average tweet length
	#the end of the user 19 features, we missed :listed_count, status retweet and focus of useron topic but if we have the user profile we can connect the two dataset and extract the needed features.
